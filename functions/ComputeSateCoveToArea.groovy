...
@TupleConstructor
class ComputeSateCoveToAreaReq {
...
@Slf4j
class ComputeSateCoveToArea {


    FunctionResultVO<List<ComputeSateCoveToAreaVO>> handle(String tle1, String tle2, List<Double> targetLon, List<Double> targetLat) {
        ...
        url = "http://192.168.11.107:8088/api/v1/satellite/computeSateCoveToArea"
        var req = new ComputeSateCoveToAreaReq()
        req.setSatOrbitDataList(positions)
        List<Target> targets = new ArrayList<>()
        for (int i = 0; i < targetLon.size(); i++) {
            targets.add(new Target(targetLat[i], targetLon[i]))
        }
        req.setAreaTarget(targets)
        var result = HttpUtil.postJson(url, new HashMap<String, String>(), req, new TypeReference<BaseSatelliteResp<List<ComputeSateCoveToAreaVO>>>() {
        })
