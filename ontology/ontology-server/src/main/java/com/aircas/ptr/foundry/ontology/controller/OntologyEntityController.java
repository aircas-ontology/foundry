...
public class OntologyEntityController {
    ...
    @Operation(summary = "根据多个本体下多个实体id查询实体主属性信息")
    @PostMapping("/query")
    ...
    @Operation(summary = "实体分页查询（只返回实体主属性信息列表）")
    @GetMapping("/list")
    public RestResult<Page<EntityInfoVO>> getEntities(
            @RequestParam(required = true, name = "ontologyUniqueIdentifier") @Parameter(description = "本体id") @OntologyIdVerify String ontologyUniqueIdentifier,
    ...
    @Operation(summary = "实体搜索（只返回实体主属性信息列表）")
    @PostMapping("/search")
    ...
    @PostMapping("/detail")
    @Operation(summary = "根据实体id查询所有属性（按列返回，动态属性默认id倒序展示前10条）")
    ...
    @Operation(summary = "根据实体id查询所有属性（按行返回，每类动态属性默认展示前100条）")
    ...
    @PostMapping("/link")
    @Operation(summary = "查询单本体单个实体所有关联关系")
    ...
    @Operation(summary = "查询多本体下多个实体所有关联关系")
    ...
    @Operation(summary = "批量新增实体，返回实体主键列表")
    @PostMapping("/create")
    ...
    @Operation(summary = "根据本体关系id创建所有实体节点关系")
